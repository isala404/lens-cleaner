...
export class EmbeddingsProcessor {
	...
	async initialize(): Promise<void> {
		...
		try {
			...
			if (!featureExtractor) {
				...
				featureExtractor = (await pipeline('image-feature-extraction', 'Xenova/dinov2-base', {
					device: 'webgpu'
	 ...
	 */
	async calculateEmbedding(base64Image: string): Promise<Float32Array> {
		if (!this.initialized) {
		...
		try {
			...
			if (output.data) {
				...
				embedding = new Float32Array(output as unknown as number[]);
			}
	...
	normalizeEmbedding(embedding: Float32Array | number[]): Float32Array {
		...
		// Calculate L2 norm
		const norm = Math.sqrt(array.reduce((sum, val) => sum + val * val, 0));
