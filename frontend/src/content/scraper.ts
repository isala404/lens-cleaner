...
async function selectPhotosForDeletion(photoIds: string[]) {
  ...
  while (staleScrollCount < maxStaleScrolls && selectedCount < photoIds.length) {
      ...
      if (match && match[1]) {
        ...
        if (photoIds.includes(photoId)) {
          ...
          if (checkbox) {
            checkbox.click();
            selectedCount++;
            updateDeletionProgress(`Selecting photos (${selectedCount}/${photoIds.length})...`, selectedCount);
            await sleep(100);
          }
        }
      }
    ...
    // Check if we made progress
    if (selectedCount === initialCount) {
      staleScrollCount++;
    } else {
      staleScrollCount = 0;
    }

    // Stop if we selected all photos
    if (selectedCount >= photoIds.length) {
      break;
    }
